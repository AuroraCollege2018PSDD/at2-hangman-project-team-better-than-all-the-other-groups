...
WORD_TEXT_SIZE = 80 #text size variable we can refer to text rendering
...
loseSound = P.mixer.Sound('media/explosion.wav')
...
def loseText():
    loseArray = [] #empty array for the letters to be rendered
    loseSound.play()# plays the loosing explosion sound
    for letter in loseWord: 
        rLetter = renderedLetter(letter)
    ...
    for l in loseArray: #follows the same steps as the last
        l.x = xPosition #set the x position for an individual rendered letter
        l.y = yPosition #set the y position for an individual rendered letter
        l.color = black
...
def lifeImages(): # sets the images for the lives
    ...
    image = P.transform.scale(image,(500,325)) #scales the image
    screen.blit(image, (500,25)) # displays the image
    ...
    for event in P.event.get():  # gets any interactions the user has made
            if event.type == P.QUIT:  # if the X in the top corner has been clicked, the game will exit
                ...
                if lives <= 0: # breaks the loop if there are no lives
                    ...
                    loseText()#refers back to anohter part of the code to render out the text
                    P.display.flip()
                    T.sleep(5)
                    
                    play = False
                elif winCount == len(word):
                    winScreen()
                    P.display.flip()
                    winSound.play()
                    T.sleep(5)
